  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const segmentsResponse = await fetch("http://localhost:8080/users");
        const segmentsData = await segmentsResponse.json();
        const uniqueSegments = Array.from(new Set([...uniqueSegmentsUsers, ...segmentsData.map(segment => segment.segment)]));
